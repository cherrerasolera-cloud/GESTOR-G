  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white mb-2">Calculadora de Logística Colaborativa</h1>
          <p className="text-gray-400">Optimiza costos compartiendo recolecciones con establecimientos cercanos.</p>
        </div>
        
        {stats.isViable && (
          <button className="flex items-center gap-2 bg-secondary text-black font-bold px-6 py-3 rounded-lg hover:bg-yellow-400 transition-colors shadow-lg shadow-yellow-500/20 animate-in fade-in">
            <Truck size={18} />
                    ...
                    <div className="flex justify-between items-start z-10 relative">
                      <div className="flex items-center gap-3">
                         <div className={`w-4 h-4 rounded border flex items-center justify-center
                           ${isSelected ? 'bg-secondary border-secondary' : 'border-gray-500'}
                         `}>
                         ...
                         <div>
                           <div className="text-sm font-medium text-white flex items-center gap-2">
                             {gen.name}
                             {isSelf && <span className="text-[10px] bg-primary/20 text-primary px-1.5 rounded">TÚ</span>}
                           </div>
                           <div className="text-xs text-gray-400">{gen.address} • {gen.distanceKm}km</div>
                         </div>
                      </div>
                      <div className="text-right">
                        <div className={`font-mono font-bold ${gen.currentLoadKg < 20 ? 'text-orange-400' : 'text-emerald-400'}`}>
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
             ...
             <div className={`border rounded-xl p-4 flex flex-col justify-between
                ${!stats.isViable ? 'bg-red-500/10 border-red-500/30' : 'bg-surface border-border'}
             `}>
               <div className="flex items-start justify-between mb-2">
                 <span className="text-gray-400 text-xs uppercase font-medium">Estado de Recolección</span>
                 {!stats.isViable ? <AlertTriangle className="text-red-500" size={20} /> : <CheckCircle2 className="text-emerald-500" size={20} />}
               </div>
               
               {!stats.isViable ? (
                 <div>
                   <div className="text-red-400 font-bold text-sm mb-1">Volumen Insuficiente</div>
                   <p className="text-xs text-gray-400 leading-tight">Tienes &lt;20kg. Selecciona un aliado con &gt;20kg para habilitar la ruta.</p>
                 </div>
               ) : (
                 <div>
                   <div className="text-white font-bold text-xl">{stats.totalKg} kg</div>
                   <p className="text-xs text-emerald-400">Volumen Asegurado</p>
                 </div>
               )}
             </div>
             ...
             <div className="bg-surface border border-border rounded-xl p-4 flex flex-col justify-between">
                <div className="flex items-start justify-between mb-2">
                  <span className="text-gray-400 text-xs uppercase font-medium">Ahorro Estimado</span>
                  <DollarSign className="text-secondary" size={20} />
                </div>
                <div>
                   <div className="text-white font-bold text-xl">${stats.savings.toFixed(2)} USD</div>
                   {stats.count > 2 ? (
                     <span className="inline-block text-[10px] px-2 py-0.5 rounded bg-secondary/20 text-secondary border border-secondary/30">
             ...
             <div className="bg-surface border border-border rounded-xl p-4 flex flex-col justify-between">
                <div className="flex items-start justify-between mb-2">
                  <span className="text-gray-400 text-xs uppercase font-medium">Impacto Evitado</span>
                  <Leaf className="text-primary" size={20} />
                </div>
                <div>
                   <div className="text-white font-bold text-xl">{stats.co2Reduction.toFixed(1)} kg CO2</div>
          ...
          <div className="relative">
             ...
             <div className="absolute top-4 right-4 bg-black/60 backdrop-blur border border-border p-3 rounded-lg max-w-[200px]">
                <div className="text-xs text-gray-300 mb-1">Ruta Sugerida</div>
