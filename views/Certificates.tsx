    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between md:items-end gap-4">
        ...
        <div className="flex bg-surface rounded-lg p-1 border border-border">
          <button 
            ...
            className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'repository' ? 'bg-secondary/20 text-secondary' : 'text-gray-400 hover:text-white'}`}
          >
            Reporte Mensual
          </button>
          <button 
            onClick={() => setActiveTab('profile')}
            className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'profile' ? 'bg-secondary/20 text-secondary' : 'text-gray-400 hover:text-white'}`}
          >
            Datos del Generador
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          {activeTab === 'repository' && (
            <div className="bg-surface border border-border rounded-xl overflow-hidden">
               <div className="p-4 border-b border-border flex justify-between items-center bg-white/5">
                 ...
                 <button className="text-xs flex items-center gap-1 text-primary hover:text-emerald-300 border border-primary/20 px-2 py-1 rounded bg-primary/5 transition-colors">
               ...
               <div className="overflow-x-auto">
                 <table className="w-full text-sm text-left">
                    ...
                    <tbody>
                      {reports.map((report) => (
                        <tr key={report.month} className="border-b border-border hover:bg-white/5 transition-colors">
                          <td className="px-6 py-4 font-medium text-white">{report.month}</td>
                          <td className="px-6 py-4 font-mono text-gray-300">
                            {report.kgGenerated !== null ? (
                                <span className="text-white font-bold">{report.kgGenerated} kg</span>
                            ) : '-'}
                          </td>
                          <td className="px-6 py-4">
                              {report.fileUrl ? (
                                  ...
                                  <span className="text-gray-600 text-xs italic">No cargado</span>
                              )}
                          </td>
                          <td className="px-6 py-4">
             ...
             <div className="space-y-6">
                ...
                <div className="bg-surface border border-border rounded-xl p-6">
                    <div className="flex items-center gap-3 mb-6 pb-4 border-b border-border">
                       ...
                       <div>
                           <h3 className="text-lg font-semibold text-white">Información del Establecimiento</h3>
                           <p className="text-sm text-gray-400">Datos registrados ante la autoridad y contacto operativo.</p>
                       </div>
                    </div>
                    ...
                    <div className="mt-6 pt-6 border-t border-border">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                             <div className="bg-background p-3 rounded-lg border border-border">
                                 ...
                                 <div className="text-white font-medium">{MOCK_PROFILE.encargadoPlataforma}</div>
                             ...
                             <div className="bg-background p-3 rounded-lg border border-border">
                                 ...
                                 <div className="text-white font-medium">{MOCK_PROFILE.contactoOperativo}</div>
                                 <div className="text-xs text-gray-400 mt-1">{MOCK_PROFILE.emailContacto}</div>
                                 <div className="text-xs text-gray-400">{MOCK_PROFILE.telefonoContacto}</div>
                             </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-surface border border-border rounded-xl p-6 relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-3 mb-6">
                       ...
                       <div>
                           <h3 className="text-lg font-semibold text-white">Plan de Mantenimiento Sugerido</h3>
                           <p className="text-sm text-gray-400">Basado en tu volumen de residuos y tipo de trampa.</p>
                       </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                         <div className="bg-background border border-border p-4 rounded-lg">
                             ...
                             <div className="text-white font-bold">{MOCK_PROFILE.tipoTrampa}</div>
                             <div className="text-xs text-gray-400 mt-1">Capacidad: {MOCK_PROFILE.capacidadTrampaLts} Litros</div>
                         </div>
                         <div className="bg-background border border-border p-4 rounded-lg">
                             <span className="text-xs text-gray-500 block mb-1">Frecuencia Limpieza</span>
                             <div className="text-white font-bold">Cada {MOCK_PROFILE.frecuenciaMantenimientoDias} días</div>
                             <div className="text-xs text-emerald-400 mt-1">Recomendada</div>
                         </div>
                         <div className={`border p-4 rounded-lg flex flex-col justify-center ${
                             ...
                             <span className={`text-xs font-bold uppercase mb-1 ${
                                 maintenanceStatus.isOverdue ? 'text-red-400' : maintenanceStatus.isDueSoon ? 'text-amber-400' : 'text-emerald-400'
                             }`}>
                                 {maintenanceStatus.isOverdue ? '¡Vencido!' : 'Próxima Limpieza'}
                             </span>
                             <div className="text-white font-bold text-lg">{maintenanceStatus.next}</div>
                    ...
                    {maintenanceStatus.isDueSoon && (
                        <div className="flex items-center gap-2 p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg text-amber-200 text-sm mb-4 animate-in fade-in">
                    ...
                    <div className="flex justify-end">
                        <button className="text-sm bg-secondary/10 text-secondary border border-secondary/20 hover:bg-secondary/20 px-4 py-2 rounded-lg transition-colors font-medium flex items-center gap-2">
                ...
                <div className="bg-surface border border-border rounded-xl p-6">
                    <div className="flex justify-between items-start mb-6">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="text-lg font-semibold text-white">Caracterización de Aguas</h3>
                                <p className="text-sm text-gray-400">Monitoreo de vertimientos y cumplimiento de parámetros.</p>
                            </div>
                        </div>
                        <button className="flex items-center gap-2 text-xs bg-purple-500/10 text-purple-300 border border-purple-500/20 hover:bg-purple-500/20 px-3 py-1.5 rounded transition-colors">
                            <Upload size={14} />
                                ...
                                <div className="flex flex-col md:flex-row justify-between md:items-center mb-4 gap-4">
                                    <div>
                                        <div className="flex items-center gap-2 mb-1">
                                            <h4 className="text-white font-medium">Informe: {report.date}</h4>
                                            {report.status === 'Compliant' ? (
                                                <span className="px-2 py-0.5 bg-emerald-500/10 text-emerald-400 text-[10px] rounded border border-emerald-500/20 uppercase font-bold">Cumple</span>
                                            ) : (
                                                <span className="px-2 py-0.5 bg-red-500/10 text-red-400 text-[10px] rounded border border-red-500/20 uppercase font-bold">No Cumple</span>
                                            )}
                                        </div>
                                    ...
                                    <div className="flex gap-4">
                                        <div className="text-center">
                                            ...
                                            <div className={`font-mono font-bold ${report.ph < 6 || report.ph > 9 ? 'text-red-400' : 'text-white'}`}>{report.ph}</div>
                                        </div>
                                        <div className="text-center">
                                            <div className="text-[10px] text-gray-500 uppercase">Grasas (mg/L)</div>
                                            <div className={`font-mono font-bold ${report.grasasAceitesMgL > 100 ? 'text-red-400' : 'text-white'}`}>{report.grasasAceitesMgL}</div>
                                        </div>
                                        <div className="text-center">
                                            <div className="text-[10px] text-gray-500 uppercase">Sólidos (mg/L)</div>
                                            <div className="text-white font-mono font-bold">{report.solidosSuspendidosMgL}</div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {report.recommendations.length > 0 && (
                                    <div className="mt-3 pt-3 border-t border-border/50">
                                        <div className="flex items-start gap-2">
                                            ...
                                            <div>
                                                <span className="text-xs text-yellow-400 font-bold uppercase block mb-1">Recomendaciones del Sistema</span>
        ...
        <div className="lg:col-span-4 space-y-6">
           ...
           {activeTab === 'profile' && (
             <div className={`border rounded-xl p-4 flex flex-col gap-3 ${isRegistered ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-red-500/5 border-red-500/20'}`}>
               <div className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${isRegistered ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'}`}>
                      {isRegistered ? <ShieldCheck size={18} /> : <XCircle size={18} />}
                  </div>
                  <h3 className={`font-bold text-sm ${isRegistered ? 'text-emerald-400' : 'text-red-400'}`}>
               ...
               {!isRegistered && (
                  <button className="w-full py-2 bg-red-500 text-white text-xs font-bold rounded hover:bg-red-600 transition-colors">
                     Iniciar Inscripción
                  </button>
               )}
             </div>
              ...
              <div className="space-y-3">
                 {COMPLIANCE_CHECKLIST.map((item) => (
                    <div key={item.id} className="flex items-start gap-3 p-2 rounded hover:bg-white/5 transition-colors">
                       <div className={`mt-0.5 w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${item.isComplete ? 'bg-primary text-black' : 'bg-background border border-gray-600'}`}>
                          {item.isComplete && <CheckCircle2 size={14} />}
                       </div>
                       <div>
                          <p className={`text-sm ${item.isComplete ? 'text-gray-300' : 'text-gray-500'}`}>{item.label}</p>
                          <span className="text-[10px] uppercase tracking-wider text-gray-600 bg-black/40 px-1.5 py-0.5 rounded">{item.requiredFor}</span>
                       </div>
                    </div>
                 ))}
              </div>
           ...
           <div className="bg-gradient-to-br from-yellow-900/40 to-surface border border-yellow-500/20 rounded-xl p-5">
              <h4 className="text-yellow-400 font-medium mb-2 text-sm">¿Necesitas ayuda?</h4>
              <p className="text-xs text-gray-400 mb-3">
                 Si tienes dudas sobre el reporte a la autoridad ambiental, contacta a soporte normativo.
              </p>
              <button className="w-full py-2 bg-yellow-500/10 border border-yellow-500/30 text-yellow-400 rounded-lg text-xs hover:bg-yellow-500/20 transition-colors">
                 Contactar Soporte
              </button>
           </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
           <div className="bg-surface border border-border rounded-xl w-full max-w-md p-6 shadow-2xl">
              <div className="flex justify-between items-center mb-6">
                 <h3 className="text-lg font-bold text-white">Editar Mes: {selectedReport.month}</h3>
                 <button onClick={() => setSelectedReport(null)} className="text-gray-500 hover:text-white"><X size={20} /></button>
              </div>
              
              <form onSubmit={handleUpdateReport} className="space-y-4">
                 <div>
                    <label className="block text-sm text-gray-400 mb-1">Corrección de Cantidad (Kg)</label>
                    <input 
                      type="number" 
                      required
                 ...
                 <div className="pt-4 flex gap-3">
                    <button type="button" onClick={() => setSelectedReport(null)} className="flex-1 py-2.5 bg-background border border-border rounded-lg text-gray-300 hover:text-white transition-colors">Cancelar</button>
                    <button type="submit" className="flex-1 py-2.5 bg-primary text-black font-semibold rounded-lg hover:bg-emerald-400 transition-colors">Guardar Cambio</button>
                 </div>
              </form>
           </div>
        </div>
